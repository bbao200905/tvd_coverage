...
class TVD(CoverageControl):
    ...
    def calculateVelocities(self, x=None):
        ...
        grad1 = np.linalg.inv(self.A) @ (self.b)
    ...
    def calculateTVDTerms(self):
        ...
        self.dcdp = np.zeros([self.n_agents * self.dim, self.n_agents * self.dim])
        for i, partition in enumerate(self.vor):
            [neighbors, vertices, p_neigh] = tvd.return_neighbors(
                self.vor, i, self.env
            )
...
class TVD_SP(TVD):
    ...
    def __singularPerturbation(self):
        ...
        if self.algorithm == "TVD-SP":
            ...
            raise ValueError(
                "Algorithm choices are 'TVD-SP', 'TVD-SSP', 'TVD-SP-hybrid', 'TVD-SP-delayed'"
            )
    ...
    def sing_perturbation_delayed(
        ...
        if debug_mode:
            raise NotImplementedError(
                "See tvd_calc_2d.py for plots, debugging, and comparison to normal sing_perturbation"
            )
        ...
        for step in range(n_steps):
            ...
            u_next = (
                u - deta * grad
            )  # idk why the method does not work here. Rasises type error.
            u_past = u
            u = u_next
            if step % interval == 0:
    ...
    def sing_perturbation_hybrid(
        ...
        n_steps = int(np.ceil(t_int / deta))
        if debug_mode:
            raise NotImplementedError("Use tvd_calc_2d.py to see eigenvalues")
        u_hybrid_vec = []  # Hybrid
